...
public class ImageSeamAssistant
{
    ...
    public ImageSeamAssistant(Mat source, Mat weighted, int percentage)
    {
        Source = source;
        Weighted = weighted;
    ...
    public Mat ScaleX()
    {
        ...
        var slices = (int)(cols.SeamList.Count * (_percentage / 100f));

        var slice = cols.SeamList.GetRange(0, slices);

        foreach (var seam in slice) ShiftColToLeft(seam);
    ...
    public Mat ScaleY()
    {
        ...
        var slices = (int)(rows.SeamList.Count * (_percentage / 100f));

        var slice = rows.SeamList.GetRange(0, slices);

        foreach (var seam in slice) ShiftRowToTop(seam);
    ...
    private Seams GetRowSeams()
    {
        ...
        foreach (var row in Enumerable.Range(0, Source.Rows)) rows.SeamList.Add(GetRowSeam(row));
    ...
    private Seams GetColSeams()
    {
        ...
        foreach (var col in Enumerable.Range(0, Source.Cols)) cols.SeamList.Add(GetColSeam(col));
